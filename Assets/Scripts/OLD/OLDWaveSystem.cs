...
[Serializable]
public class OLDEnemyData
{
    public OLDEnemyType type;
    public GameObject prefab;
}

public class OLDWaveSystem : MonoBehaviour
{
    [SerializeField] List<OLDEnemyData> m_enemies = new List<OLDEnemyData>();
    ...
    string SetDescriptionFromWaves(List<int> wave)
    {
        ...
        for(int i = 0; i < wave.Count; i++)
        {
            ...
            description += GetName((OLDEnemyType)i);
            description += " : ";
    ...
    private void Start()
    {
        int nbType = Enum.GetValues(typeof(OLDEnemyType)).Length;
    ...
    OLDEnemyData GetEnemy(OLDEnemyType type)
    {
    ...
    string GetName(OLDEnemyType type)
    {
        switch(type)
        {
            case OLDEnemyType.fast:
                return "Rapide";
            case OLDEnemyType.heavy:
                return "Lourd";
            case OLDEnemyType.kamikaze:
                return "Kamikaze";
            case OLDEnemyType.normal:
                return "Normal";
            case OLDEnemyType.sniper:
                return "Sniper";
        }
    ...
    void SpawnOneEnemy()
    {
        ...
        OLDEnemyType type = OLDEnemyType.normal;
        for (int i = 0; i < m_remainingEnemies.Count; i++)
        {
        ...
        OLDEnemyData data = GetEnemy(type);
        if (data == null)
            return;
    ...
    void GenerateSpawnPos()
    {
        ...
        for(int i = 0; i < nbPos; i++)
        {
            ...
            Vector3 pos = OLDWorldHolder.Instance().GetElemPos(x, y);
            pos.y += 1.2f;
