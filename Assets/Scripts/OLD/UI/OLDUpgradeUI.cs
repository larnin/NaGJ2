...
public class OLDUpgradeUI : MonoBehaviour
{
    Canvas m_canvas;
    ...
    private void Start()
    {
        ...
        int level = 0;
        var building = OLDWorldHolder.Instance().GetBuilding(m_pos.x, m_pos.y, out level);

        m_label.text = "Level " + (level + 1).ToString();

        int nbLevel = OLDElementHolder.Instance().GetMaxBuildingLevel(building);
        if(nbLevel <= level + 1)
        {
        ...
        else
        {
            ...
            int cost = OLDElementHolder.Instance().GetBuildingCost(building, level + 1);
    ...
    void OnButtonClick()
    {
        ...
        if (nbLevel > level + 1)
        {
            ...
            if (e.money >= cost)
                OLDGameSystem.Instance().PlaceTower(m_pos.x, m_pos.y, building, level + 1);
        }
    ...
    {
        //pos
        var pos = OLDWorldHolder.Instance().GetElemPos(m_pos.x, m_pos.y);
